<template>
  <div id="guide" style="margin: 0 auto;" v-if="guide">
    ...
    <div style="text-align: center;">
      ...
      <p>
        <n-button class="button" color="#8a2be2" style="font-size: 2em;" @click="connect">连接设备</n-button>
        <n-button class="button" color="#8a2be2" style="font-size: 2em;" @click="start">开始采集</n-button>
      </p>
      <p style="color: #ffffff;">{{ (connected)?'连接成功':(stage>=1)?'连接中':'未连接' }}</p>
    </div>
  </div>
...
export default {
  data() {
    ...
    return { token, port, recvBuffer, stage, guide, show, ctx, id, eegdata, eegdata_pkg, eegdata_step, post_pid, eegdata_post, connected, video_options, video_playing, video_resolve, ctx3 };
  },
  methods: {
    ...
    async getToken() {
      const res = await fetch('http://127.0.0.1:3000/getToken');
      const result = await res.json();
      return result.token;
    },
    async logEvent(event) {
      const res = await fetch(`http://127.0.0.1:3000/event?token=${this.token}`, {
        method: 'POST',
    ...
    async saveEEG(pid, data, end = false) {
      const res = await fetch(`http://127.0.0.1:3000/saveEEG?token=${this.token}&pid=${pid}${end?'&end=true':''}`, {
        method: 'POST',
                ...
                if(this.stage <= 5) {
                  for(const i of value) {
                    ...
                    if(this.recvBuffer[this.recvBuffer.length - 2] === 13 && this.recvBuffer[this.recvBuffer.length - 1] === 10) {
                      ...
                      if(this.stage === 3 && recv.startsWith('+ADVADDR') && recv.split(',')[3]) {
