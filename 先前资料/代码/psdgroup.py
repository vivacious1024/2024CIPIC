    ...
    for root, dirs, files in os.walk(path, topdown=False):
        for file in files:
            if '.mat' in file:
                ...
                if not os.path.exists('F:\作业\本基项目\\psd\\' + dirname[-2] + '\\' + dirname[-1]):
                    os.makedirs('F:\作业\本基项目\\psd\\' + dirname[-2] + '\\' + dirname[-1])
                scipy.io.savemat('F:\作业\本基项目\\psd\\' + dirname[-2] + '\\' + dirname[-1] + '\\' + file,
                                 {'epoch': eegpsd})
                count += 1


def psd_save(path='F:\作业\本基项目\group'):
...
def psd_com_part(path):
    ...
    for dirr in os.listdir(path):
        ...
        if not os.path.exists(target+dirr):
            os.makedirs(target+dirr)
        positives = getpsd(source+'1')
        neuters = getpsd(source + '2')
        negatives = getpsd(source + '3')
...
if __name__ == "__main__":
    ...
    """
    # psd_save()
    # psd_com('F:\作业\本基项目\\psd')
    psd_com_part('F:\作业\本基项目\\psd')