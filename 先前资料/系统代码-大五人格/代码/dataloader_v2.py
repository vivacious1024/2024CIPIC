...
class Dataset_eeg(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        data = data['epoch']
        dataout = np.empty((3,10,610))
        for i in range(data.shape[0]):
            data[i,:] = (data[i,:] - np.mean(data[i,:])) / np.std(data[i,:])
            if i<10:
                dataout[0,i,:] = data[i,:]
            elif i>=10 and i<20:
                dataout[1,i-10,:] = data[i,:]
            else:
                dataout[2,i-20,:] = data[i,:]
#         data = np.reshape(data, (150,122))
        data = np.reshape(data, (3,100,61))
